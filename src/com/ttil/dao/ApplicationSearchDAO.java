...
public class ApplicationSearchDAO {
	...
	/**
	 * This will  fetch basic details for Forgot Password Users with Registration Id & DOB
	...
	public ApplicationFormBean getPasswordDeatils(ApplicationSearchBean applicationSearchBean)
	{
		...
		try{
			...
			if(con!=null)
			{
				query="select u.username,u.password,a.transactionid,a.post_applied,a.post_selected_name,a.mobilenumber,a.emailaddress,a.first_name, a.middle_name, a.last_name,a.application_status,a.dob,a.community,a.sex from userinfo u,applicants a WHERE u.transactionid=a.transactionid and a.application_status is not null and  a.dob='"+StringUtils.changeDateFormatYMD(ESAPI.encoder().canonicalize(applicationSearchBean.getDob()))+"' and a.transactionid='"+applicationSearchBean.getTransactionid()+"'";
				...
				//LogsGeneration.generateErrorLogsWithMobileNumber("SearchDataDAO", "Connection is null", null, null, null);
				return null;
			}
		}catch(Exception e)
		{
			...
			//LogsGeneration.generateErrorLogsWithMobileNumber("SearchDataDAO in Catch", exceptionAsString, null, null, null);
			return null;
		}
		finally{
			try{
			...
			catch(Exception e)
			{
				...
				e.printStackTrace(new PrintWriter(sw));
				String exceptionAsString = sw.toString();
				//LogsGeneration.generateErrorLogsWithMobileNumber("SearchDataDAO in finally",exceptionAsString, null, null, null);
			}
		}
	}

	/**
	 * This will fetch basic details for Forgot Registratin ID Fetching with post Applied, Mobile number & DOB
	 * @param applicationSearchBean
	 * @return
	 */
		...
		try{
			...
			if(con!=null)
			{
				...
				if(rs.next())
				{
					...
					}else if("PHOTO NOT UPLOADED".equalsIgnoreCase(applicationFormBean.getApplication_final_status())){
					...
					if("Finished".equalsIgnoreCase(applicationFormBean.getApplication_status())){
						if("Success".equalsIgnoreCase(applicationFormBean.getPayment_status()) || applicationFormBean.isPayExempted()){
							if(applicationFormBean.getPost_applied()!=null && applicationFormBean.getPost_applied().equalsIgnoreCase("9")){
					...
					if(applicationFormBean.getAffected_categories()!=null)
					{
						...
						StringTokenizer st=new StringTokenizer(applicationFormBean.getAffected_categories(),",");
						while (st.hasMoreElements()) {
							affected_category_names_sb.append(StringUtils.getAffectedCategoryName(st.nextElement().toString()));
						...
						{
							affected_category_names=affected_category_names.substring(0,affected_category_names.lastIndexOf(","));
						}
					}
					...
					query_check="select * from debarment_details where transactionid='"+applicationFormBean.getTransactionid()+"'";
					...
					query_check="select * from fir_details where transactionid='"+applicationFormBean.getTransactionid()+"'";
					...
					query_check="select * from service_details where transactionid='"+applicationFormBean.getTransactionid()+"'";
					...
					query_check="select * from exman_details where transactionid='"+applicationFormBean.getTransactionid()+"'";
					...
					while (personal.next()) {
						Object registered_nursing_council = personal.getObject("registered_nursing_council");
						...
						Object draughtsmanship_experience = personal.getObject("draughtsmanship_experience");
					...
					if(applicationFormBean.isCategoty_belongs()){
						String certificate_query="select * from certificate_details where transactionid='"+applicationFormBean.getTransactionid()+"' and certificate='Community'";
					...
					if(applicationFormBean.isJk_domiciled()){
						String certificate_query="select * from certificate_details where transactionid='"+applicationFormBean.getTransactionid()+"' and certificate='JK Domicile'";
						pstmt2=con.prepareStatement(certificate_query);
						rs2=pstmt2.executeQuery();
						while(rs2.next())
						{
					...
					if(applicationFormBean.isRiots_affected()){
						String certificate_query="select * from certificate_details where transactionid='"+applicationFormBean.getTransactionid()+"' and certificate='Riots 1984'";
						pstmt2=con.prepareStatement(certificate_query);
						rs2=pstmt2.executeQuery();
						while(rs2.next())
						{
					...
					if(applicationFormBean.isRiots_gujarat()){
						String certificate_query="select * from certificate_details where transactionid='"+applicationFormBean.getTransactionid()+"' and certificate='Riots 2002 Gujarat'";
						pstmt2=con.prepareStatement(certificate_query);
						rs2=pstmt2.executeQuery();
						while(rs2.next())
						{
					...
					query_check="select * from education_details where transactionid='"+applicationFormBean.getTransactionid()+"'";
					...
					query_check="select * from other_skills where transactionid='"+applicationFormBean.getTransactionid()+"'";
					pstmt2=con.prepareStatement(query_check);
					rs2=pstmt2.executeQuery();
