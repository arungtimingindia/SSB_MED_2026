 ...
 * In ApplicationSave Servlet after checking Data in DB will return flag
  if it returns any flag less than 0 then redirects to Preview page other wise Data will save into database and 
 * SMS will be sent asynchronously and Redirects to UploadPhotoSignature Page
 */
	...
	public void service(HttpServletRequest request,HttpServletResponse res) throws ServletException,IOException
	{
		...
		try{
			...
			if (session!=null && session.isNew()){
				...
				if(requestFrom!=null && "applicationSave".equalsIgnoreCase(requestFrom)) // Checking weather request is correct or not 
				{
					...
					if(applicationFormBean!=null && applicationFormBean.getMobileNumber()!=null) // Checking Bean object
					{ 
						...
						if(transaction_id<0){
							if(transaction_id==-4){
								...
								if(transaction_id==-3){
									applicationFormBean.setMessage("Mobile number & Email address already registered for this Advertisement");
								}
								if(transaction_id==-1){
									applicationFormBean.setMessage("Mobile number already registered for this Advertisement");
								}
								if(transaction_id==-2){
									applicationFormBean.setMessage("Email address already registered for this Advertisement");
								}
								if(transaction_id==-5){
									applicationFormBean.setMessage1("Your details (Name, Fathername, Mothername,DOB) already registered for this Advertisement");
								}
							}
						...
						else
						{
							...
							int final_transaction_id=applicationFormDAO.saveData(applicationFormBean,request.getRemoteAddr(),1,1,request.getHeader("User-Agent"));
							...
							if(final_transaction_id>0)
							{
								...
								if(applicationFormBean.isPayExempted()) {
									asyncContext.start(new AsyncOTPRunner(asyncContext,applicationFormBean.getMobileNumber(),"You Successfully Applied for Recruitment of "+postShortName+" under "+applicationFormBean.getCommunity()+" Cat. with Reg Id:"+applicationFormBean.getTransactionid()+" and password:"+ESAPI.encoder().canonicalize(applicationFormBean.getPassword())+", please upload photo and sign- SSB",request.getRemoteAddr(),request.getHeader("User-Agent"),session.getId()));
									//asyncContext.start(new CreateRegMailTemplateSB(asyncContext,applicationFormBean,request.getRemoteAddr(),request.getHeader("User-Agent"),session.getId()));
								}else {
									asyncContext.start(new AsyncOTPRunner(asyncContext,applicationFormBean.getMobileNumber(),"You Successfully Applied for Recruitment of "+postShortName+" under "+applicationFormBean.getCommunity()+" Cat. with Reg Id:"+applicationFormBean.getTransactionid()+" and password:"+ESAPI.encoder().canonicalize(applicationFormBean.getPassword())+", please pay fee- SSB",request.getRemoteAddr(),request.getHeader("User-Agent"),session.getId()));
									//asyncContext.start(new CreateRegMailTemplateSB(asyncContext,applicationFormBean,request.getRemoteAddr(),request.getHeader("User-Agent"),session.getId()));
