...
<%@ page import="com.ttil.util.StringUtils" %>

<%ApplicationFormBean afb = (ApplicationFormBean) session.getAttribute("ApplicationFormBean");
 
boolean categotyBelongs = afb.isCategoty_belongs();
boolean exserviceman = afb.isEx_serviceman();
boolean riotsAffected = afb.isRiots_affected();
boolean employeeSsb = afb.isEmployee_ssb();
boolean debarrment = afb.isDebarment();
boolean firCases=afb.isFir_cases();
boolean pendingCase=afb.isFir_cases_pending();
boolean arrested=afb.isArrested();
boolean acquitted=afb.isCriminal_case_acquitted();
boolean goodBehaviour=afb.isGood_behavior_bond();
int postApplied=Integer.parseInt(afb.getPost_applied());
%>
...
// alert(2.1)
		...
		*/	
		...
		$("input:radio[name=categoty_belongs]").on("change", function() { 
		    var value = $(this).val(); 
		    if (value === "true") {
		        $("#catdetailsdiv").show();
		    } else {
		...
		$("input:radio[name=riots_affected]").on("change", function() {
			var value = $(this).val();
			if (value == "true")
				$("#riotsdetailsdiv").show();
			else
		...
		$("input:radio[id=exservicemanid]").click(function() {
		 ...
		 $("input:radio[id=exservicemanid]").on("change", function() {
				var value = $(this).val();
				if (value == "true")
					$("#exdetailsdiv").show();
				else
		...
		$("input:radio[id=debarmentid]").on("change", function() {
			var value = $(this).val();
			if (value == "true")
				$("#debardetailsdiv").show();
			else
		...
		*/	
	    ...
	    $("input:radio[id=employee_ssb_id]").on("change",function() {
			ssbServiceText();
		});
...
function getAgeOfTwoDates(date1,date2) {
	...
	function getDistricts(){
		...
		$('#district').empty().append('<option value="">Select District</option>');
		 return $.getJSON("districts_json.txt",function(result){
		      $.each(result, function(i, field){
		    	  if(state==i){
			    	var districtsArr = field.split(",");
			        districtsArr.forEach(function(item) {
	...
	$(document).ready(function() {
		function readURL1(input) {
			if (input.files && input.files[0]) {
				...
				// check for file size
				 if (oFile.size < 20 * 1024) {
			    	document.applicationForm.photofileData.value="";
			    	alert("You have selected too small file Photo (Not less than 20KB), please select a one bigger image file");
			        return false;
			    }else if (oFile.size > 500 * 1024) {
			    	document.applicationForm.photofileData.value="";
			    	alert("You have selected too big file Photo (Not More than 500KB), please select a one smaller image file");
			        return false;
			    } else{
	...
	$(document).ready(function() {
		function readURL(input) {
			if (input.files && input.files[0]) {
				var oFile = input.files[0];
				// check for file size
				 if (oFile.size < 10 * 1024) {
			    	document.applicationForm.sigfileData.value="";
			    	alert("You have selected too small file Signature (Not less than 10KB), please select a one bigger image file");
			        return false;
			    }else if (oFile.size > 200 * 1024) {
			    	document.applicationForm.sigfileData.value="";
			    	alert("You have selected too big file Signature (Not More than 200KB), please select a one smaller image file");
			        return false;
			    } else{
      ...
      <div class="panel">
			...
			<form name=applicationForm id=applicationForm method=post
				...
				<div class="contentoutter">
					...
					<% String final_transaction_id=(String)request.getAttribute("final_transaction_id"); %>
					<h2  >
					EDIT Application Form  &nbsp;&nbsp;
					REGISTRATION NUMBER : <%=final_transaction_id %> </h2>
...
localStorage.setItem('DIS_2', dis);
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<h2>Update Photo and Signature</h2></td>
								</tr>
							</table>
								...
								<tr style="border-bottom:2pt solid black;">
									<td align="left" valign="middle"><table width="100%"
											...
											<tr>
												...
												<img id="photoblah" src="<%=StringUtils.getDecodedField(afb.getPhotoStr()) %>"  alt="" width="200" height="200"> 
												<div id="oldphoto" style="display: none;">
												<span style="font-size: 25px">&lt;-</span>
												<img  src="<%=StringUtils.getDecodedField(afb.getPhotoStr()) %>"  alt="" width="100" height="100" > 
												</div>
												</div>
												</td>
											</tr>
											<tr>
												<td><div id="file">
													...
													</div> <input name="photofileData" type="file" id="photosrc" value="Browse Photo" accept=".jpg, .jpeg, .png"/></td>
											</tr>
										</table></td>
								...
								<tr>
									<td align="left" valign="middle"><table width="100%"
											...
											<tr>
												<td>
												<div style="display: flex; align-items: center">
												<img id="sigblah" src="<%=StringUtils.getDecodedField(afb.getSignStr()) %>"  alt="" width="200" height="200"> 
												<div id="oldsign" style="display: none;">
												<span style="font-size: 25px">&lt;-</span>
												<img  src="<%=StringUtils.getDecodedField(afb.getSignStr()) %>"  alt="" width="100" height="100" > 
												</div>
												</div>
												</td>
											</tr>
											<tr>
												<td><div id="file">
														<!-- <input type="button" value="Upload Image" class="browse" /> -->
													</div> <input name="sigfileData" type="file" id="sigsrc" value="Browse Signature" accept=".jpg, .jpeg, .png"/></td>
											</tr>
...
function fillform() {
	...
	document.applicationForm.post_applied.value='<%=StringUtils.getDecodedField(afb.getPost_applied())%>';
	 $("#post_applied").trigger('change'); 
	 
	<%--  $("#post_applied").hide();
	  $("#post_selected").text('<%=StringUtils.getDecodedField(afb.getPost_selected_name())%>');
	  $("#post_selected").show(); --%>
	 
	 document.applicationForm.first_name.value='<%=StringUtils.getDecodedField(afb.getFirst_name())%>';
	  document.applicationForm.middle_name.value='<%=StringUtils.getDecodedField(afb.getMiddle_name())%>';
		document.applicationForm.last_name.value='<%=StringUtils.getDecodedField(afb.getLast_name())%>';
		...
		var caste='<%=StringUtils.getDecodedField(afb.getCommunity())%>';
		document.applicationForm.community.value=caste;
		$("#community").trigger('change')
		
		var marrital_status='<%=afb.getMarrital_status()%>';
		document.applicationForm.marrital_status.value=marrital_status;
		$("#marrital_status").trigger('change')
	...
	<% if(categotyBelongs) { %>
	 ...
	 document.applicationForm.cat_cert_no.value='<%=StringUtils.getDecodedField(afb.getCat_cert_no())%>';
		...
		document.applicationForm.cat_issue_authority.value='<%=StringUtils.getDecodedField(afb.getCat_issue_authority())%>';
	...
	<%if (riotsAffected) {%>
		...
		<%if (employeeSsb) {%>
			...
			document.applicationForm.nocdesignation.value='<%=afb.getNocdesignation()%>';
			<%}%>
			
			<% if(postApplied==1){ %>
			 document.applicationForm.hc_lab_asst_cert.value='<%=afb.isHcLabAsstCert() %>' ; 
			 document.applicationForm.hc_lab_asst_exp.value='<%=afb.isHcLabAsstExp() %>' ; 
			 <%--	document.applicationForm.pharmasist_act_exp.value='<%= afb.isPharmasist_act_exp() %>' ;  --%>
		<% } else if(postApplied==2) { %>
		
		 
		
		<% } else if(postApplied==3) { %>
		
		 document.applicationForm.hc_physio_asst_cert.value='<%=afb.isHcPhysioAsstCert() %>' ; 
		 document.applicationForm.hc_physio_asst_exp.value='<%=afb.isHcPhysioAsstExp() %>' ; 
		
		<% } else if(postApplied==4) { %>	
		 document.applicationForm.ct_nursing_asst_cert.value='<%=afb.isCtNursingCert() %>' ; 
		 document.applicationForm.ct_nursing_asst_exp.value='<%=afb.isCtNursingExp() %>' ; 
		 
		
		<% }  %>
			
			
			document.applicationForm.identity_type.value='<%=afb.getIdentity_type()%>';
			document.applicationForm.aadharNo.value='<%=afb.getAadharNo()%>';
			document.applicationForm.mobileNumber.value='<%=afb.getMobileNumber()%>';
			document.applicationForm.emailaddress.value='<%=StringUtils.getDecodedField(afb.getEmailaddress())%>';
			
			document.applicationForm.identification_mark.value='<%=StringUtils.getDecodedField(afb.getIdentification_mark())%> ';
			 
			 
			
			document.applicationForm.university_1.value='<%=StringUtils.getDecodedField(afb.getUniversity_1())%>';
		  	<%-- document.applicationForm.university_plus2.value='<%=StringUtils.getDecodedField(afb.getUniversity_2())%>';  --%>
		  	document.applicationForm.university_plus2.value='<%=StringUtils.getDecodedField(afb.getUniversity_2())%>';
			document.applicationForm.university_5.value='<%=StringUtils.getDecodedField(afb.getUniversity_plus2())%>';
			document.applicationForm.university_3.value='<%=StringUtils.getDecodedField(afb.getUniversity_3())%>';
			document.applicationForm.university_4.value='<%=StringUtils.getDecodedField(afb.getUniversity_4())%>';
			
			document.applicationForm.edu_year_1.value='<%=StringUtils.getDecodedField(afb.getEdu_year_1())%>';
			document.applicationForm.edu_year_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_year_2())%>';
		 	<%-- document.applicationForm.edu_year_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_year_2())%>';  --%>
			document.applicationForm.edu_year_5.value='<%=StringUtils.getDecodedField(afb.getEdu_year_plus2())%>';
			document.applicationForm.edu_year_3.value='<%=StringUtils.getDecodedField(afb.getEdu_year_3())%>';
			document.applicationForm.edu_year_4.value='<%=StringUtils.getDecodedField(afb.getEdu_year_4())%>';
			
			document.applicationForm.edu_rollno_1.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_1())%>';
			document.applicationForm.edu_rollno_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_2())%>';
			 <%-- document.applicationForm.edu_rollno_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_2())%>';   --%>
			document.applicationForm.edu_rollno_5.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_plus2())%>';
			document.applicationForm.edu_rollno_3.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_3())%>';
			document.applicationForm.edu_rollno_4.value='<%=StringUtils.getDecodedField(afb.getEdu_rollno_4())%>';
			
			document.applicationForm.edu_certno_1.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_1())%>';
			document.applicationForm.edu_certno_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_2())%>';
		 <%-- 	document.applicationForm.edu_certno_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_2())%>'; --%>
			document.applicationForm.edu_certno_5.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_plus2())%>';
			document.applicationForm.edu_certno_3.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_3())%>';
			document.applicationForm.edu_certno_4.value='<%=StringUtils.getDecodedField(afb.getEdu_certno_4())%>';
			
			document.applicationForm.edu_date_issue_1.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_1())%>';
			document.applicationForm.edu_date_issue_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_2())%>';
			<%--   document.applicationForm.edu_date_issue_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_2())%>';   --%>
			document.applicationForm.edu_date_issue_5.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_plus2())%>';
			document.applicationForm.edu_date_issue_3.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_3())%>';
			document.applicationForm.edu_date_issue_4.value='<%=StringUtils.getDecodedField(afb.getEdu_date_issue_4())%>';
			
			document.applicationForm.stream_1.value='<%=StringUtils.getDecodedField(afb.getStream_1())%>';
			document.applicationForm.stream_plus2.value='<%=StringUtils.getDecodedField(afb.getStream_2())%>';
		 	 <%-- document.applicationForm.stream_plus2.value='<%=StringUtils.getDecodedField(afb.getStream_2())%>';  --%> 
			document.applicationForm.stream_5.value='<%=StringUtils.getDecodedField(afb.getStream_plus2())%>';
			document.applicationForm.stream_3.value='<%=StringUtils.getDecodedField(afb.getStream_3())%>';
			document.applicationForm.stream_4.value='<%=StringUtils.getDecodedField(afb.getStream_4())%>';
			
			document.applicationForm.edu_marks_1.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_1())%>';
			document.applicationForm.edu_marks_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_2())%>';
			<%--  document.applicationForm.edu_marks_plus2.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_2())%>';  --%>
			document.applicationForm.edu_marks_5.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_plus2())%>';
			document.applicationForm.edu_marks_3.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_3())%>';
			document.applicationForm.edu_marks_4.value='<%=StringUtils.getDecodedField(afb.getEdu_marks_4())%>';
			
			document.applicationForm.village.value='<%=StringUtils.getDecodedField(afb.getVillage())%>';
			document.applicationForm.postoffice.value='<%=StringUtils.getDecodedField(afb.getPostoffice())%>';
			document.applicationForm.tehsil.value='<%=StringUtils.getDecodedField(afb.getTehsil())%>';
			document.applicationForm.policestation.value='<%=StringUtils.getDecodedField(afb.getPolicestation())%>';
			document.applicationForm.state.value='<%=StringUtils.getDecodedField(afb.getState())%>';
			

			   
			getDistricts().then(()=>{
			    document.applicationForm.district.value =
			        '<%= StringUtils.getDecodedField(afb.getDistrict()) %>';
			});  
			document.applicationForm.pincode.value='<%=StringUtils.getDecodedField(afb.getPincode())%>';
			
			
			document.applicationForm.correspondence_address.value=<%=afb.isCorrespondence_address()%>;
			var corres='<%=afb.isCorrespondence_address()%>'; 
			$('input[name=correspondence_address][value='+corres+']').prop('checked',true);  
			
			document.applicationForm.village1.value='<%=StringUtils.getDecodedField(afb.getVillage1())%>';
			document.applicationForm.postoffice1.value='<%=StringUtils.getDecodedField(afb.getPostoffice1())%>';
			document.applicationForm.tehsil1.value='<%=StringUtils.getDecodedField(afb.getTehsil1())%>';
			document.applicationForm.policestation1.value='<%=StringUtils.getDecodedField(afb.getPolicestation1())%>';
			document.applicationForm.state1.value='<%=StringUtils.getDecodedField(afb.getState1())%>';
				 
			
			getDistricts1().then(()=>{
				document.applicationForm.district1.value='<%=StringUtils.getDecodedField(afb.getDistrict1())%>';
			});
			document.applicationForm.pincode1.value='<%=afb.getPincode1()%>';
		...
		document.applicationForm.examdate.value='<%=afb.getExamdate()%>';
		document.applicationForm.debarenddate.value='<%=afb.getDebarenddate()%>';
		document.applicationForm.debar_reason.value='<%=StringUtils.getDecodedField(afb.getDebar_reason())%>';
		document.applicationForm.other_reason.value='<%=StringUtils.getDecodedField(afb.getOther_reason())%>';
				<%}%>
				document.applicationForm.fir_cases.value='<%=firCases%>';
				document.applicationForm.fir_cases_pending.value='<%=pendingCase%>';	
				document.applicationForm.arrested.value='<%=arrested%>';	
				document.applicationForm.criminal_case_acquitted.value='<%=acquitted%>';	
				document.applicationForm.good_behavior_bond.value='<%=goodBehaviour%>'; 
				
				
				 <%if(firCases||pendingCase ||arrested || acquitted||goodBehaviour){%>
				 showFirDetails();
				 document.applicationForm.firno.value='<%=StringUtils.getDecodedField(afb.getFirno())%>';
					document.applicationForm.firdate.value='<%=afb.getFirdate()%>';
					document.applicationForm.firsection.value='<%=StringUtils.getDecodedField(afb.getFirsection())%>';
